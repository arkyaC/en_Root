...
void TB(){
	...
	for(int i=0; i<n; i++){
		...
		float phi3=2*pi*rndgen->Uniform(0,1);
		float theta3=acos(rndgen->Uniform(-1,1));
		float p3x=p3*sin(theta3)*sin(phi3);
		float p3y=p3*sin(theta3)*cos(phi3);
		float p3z=p3*cos(theta3);

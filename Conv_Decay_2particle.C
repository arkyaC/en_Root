...
#define M_mother 1019.445
#define m 0.511
#define pi 3.141592653589
#define gamma_BW 4.43

void Conv_Decay_2particle(){
	TFile* ofile=TFile::Open("conv_decay.root","RECREATE");
	...
	for(int i=0;i<10000;i++){
		...
		int N=(int)(rndgen->Gaus(25.,1.));//number of phi particles
		if(i%1000==0) cout<<N<<endl;
		number_of_phi->Fill(i+1,N);
		
			
		for(int j=1;j<=N;j++){
			float M = rndgen->BreitWigner(M_mother,gamma_BW);
		    ...
		    //adding simulated error in measurement
		    Px1=rndgen->Gaus(Px1,.05*Px1);
		    Py1=rndgen->Gaus(Py1,.05*Py1);
		    Pz1=rndgen->Gaus(Pz1,.05*Pz1);
		    Px2=rndgen->Gaus(Px2,.05*Px2);
		    Py2=rndgen->Gaus(Py2,.05*Py2);
		    Pz2=rndgen->Gaus(Pz2,.05*Pz2);
